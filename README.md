...
- 录用名额事务控制：
  - 创建 Offer 要求岗位处于开放状态、候选人简历处于面试阶段；同一候选人对同一岗位仅保留一份未结束（DRAFT/APPROVED/SENT）的 Offer，由数据库部分唯一索引兜底。
  - 接受 Offer 在数据库事务内对岗位行加 `SELECT … FOR UPDATE` 行锁后核对剩余名额（`headcount - hiredCount`）：两人争抢最后一个名额时仅一人成功，成功者占用名额、简历自动转 HIRED，岗位录满后自动关闭；失败者返回 409 并在 Offer 上持久化失败原因。
  - 岗位招聘人数（headcount）不得修改为低于已录用人数；岗位关闭后不能重新开放，仅可归档。
  - 岗位详情与候选人详情展示剩余名额、已录用人数与 Offer 失败原因；面试安排与审计流程不受影响。
